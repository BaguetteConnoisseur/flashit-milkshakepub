...
$itemId = intval($_GET['id'] ?? $_POST['item-id'] ?? 0);
$feedback = null;
$item = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['save-item'])) {
        ...
        if ($itemName === '' || $description === '' || $ingredients === '' || $color === '') {
...
$itemTypeDisplay = ucfirst($item['category']);
?>
...
<head>
    ...
    <title>Redigera <?= htmlspecialchars($itemTypeDisplay) ?></title>
    <style>
        :root {
            --bg: #f3f4f6;
            --card-bg: #ffffff;
...
<body>
    ...
    <div class="container">
        <h1>Redigera <?= htmlspecialchars($itemTypeDisplay) ?></h1>
        <p class="subtitle">
        ...
        <?php endif; ?>
        <section class="card">
            <form method="post" action="/edit-item?id=<?= (int)$item['item_id'] ?>">
                <?= csrf_token_input() ?>
                <input type="hidden" name="item-id" value="<?= (int) $item['item_id'] ?>">
                <div class="row">
                    <div>
                        <label for="item-name">Namn</label>
                        <input id="item-name" type="text" name="item-name" value="<?= htmlspecialchars($item['name']) ?>" required>
                    </div>
                    <div>
                ...
                <div class="actions">
                    <button type="submit" class="btn btn-primary" name="save-item">Spara</button>
